                         ...
                         word_t **big_arr, word_t **small_arr)
{
    ...
    word_t *earray, *farray;
    earray = bigK_index(marray, page_count, k);
    farray = smallK_index(marray, page_count, k);
    if (!earray || !farray)
        goto err;
    word_t *rarray, *parray;
    rarray = auto_malloc(k * sizeof(word_t));
    parray = auto_malloc(k * sizeof(word_t));
    for (i = 0; i < k; i++)
    {
        rarray[i] = barray[earray[i]];
        parray[i] = barray[farray[i]];
    }
    *big_arr = rarray;
    *small_arr = parray;
    auto_free(barray, page_count * sizeof(word_t));
...
word_t extract_small(word_t *bitmap, word_t *infotable, word_t page_count, word_t n, word_t k, word_t **arr)
{
    ...
    if (j != page_count)
    {
        printk(KERN_ERR "j %u page_count %u\n", j, page_count);
        goto err;
    }
    word_t *earray;
    earray = smallK_index(marray, page_count, k);
    if (!earray)
        goto err;
    word_t *rarray;
    rarray = auto_malloc(k * sizeof(word_t));
...
/*
word_t wear_level(word_t *bitmap, word_t *infotable, word_t *maptable,
                  word_t *small_arr, word_t *big_arr, word_t size)
{
